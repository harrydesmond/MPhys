...
df.to_csv('/mnt/users/darnej/MPhys/TNG-50/v_sig_ratios_TNG.csv', index=False)
...
for gal in chunks:
    print(gal)
    bincenters, mean_acc_obs, mean_acc_bar, total_mean_acc_bar, v_obs, v_bar, v_tot, V_C, V_C_spline, sigma_v, gas_positions, H1_densities, rotated_points, sigma_z_only = V_plot(int(gal), ozy_file = 'hey' ,sim = 'TNG')
    ...
    stellar_mass = galaxies_dataframe['stellar_mass'][galaxies_dataframe['gal_number'] == int(gal)].values[0]
    ...
    sigma_z_only = sigma_z_only/1000


    if np.array(sigma_v).all == 0 or len(v_obs[mask]) == 0 or isinstance(sigma_v, float) or np.array(sigma_z_only).all == 0:
        mean_v_sig_ratio = 0
        mean_sig_v = 0
        max_v_rot = 0
        mean_v_rot = 0
        ...
        mean_sig_v_z = np.mean(sigma_z_only[mask])
...
if rank == 0:
    ...
    df.to_csv('/mnt/users/darnej/MPhys/TNG-50/v_sig_ratios_TNG.csv', mode='a', header=False, index=False)
