...
def V_plot(gal_number, ozy_file, sim):
    ...
    if sim == 'NH':
        ...
        Rgas_1_2 = dataframe['gas_r_half'][int(gal_number)-1]
    ...
    for i in range(number_of_bins):
        ...
        vtheta_in_bin = v_in_bin[:,1]

        mass_in_bin = h1_masses[mask]

        if len(v_in_bin) > 1:

            #CALCULATE V_OBS AND G_OBS
            ...
            z_mask = (h1_cylindrical_points[:,2] < gas_z_half) & (h1_cylindrical_points[:,2] > -gas_z_half) & (h1_cylindrical_shells > min_r_for_bin) & (h1_cylindrical_shells < max_r_for_bin)
            sigma_in_bin_z_only = get_sigma_p(h1_cylindrical_velocities[z_mask])

            sigma.append(sigma_in_bin)
            sigma_z_only.append(sigma_in_bin_z_only)
            ...
            v_bar.append(np.sqrt(mean_acc_bar[-1] * r_in_m))
            ...
            v_tot.append(np.sqrt(total_mean_acc_bar[-1] * r_in_m))
    ...
    sigma_z_only = np.array(sigma_z_only)
    ...
    sigma_z_only = np.sqrt((sigma_z_only[:,0]**2 + sigma_z_only[:,1]**2 + sigma_z_only[:,2]**2)/3)
    ...
    # You can then use these fitted_Rc and fitted_Rd values in your V_A calculation.
        ...
        VA2 = np.zeros_like(mean_r)
        VA_spline = np.zeros_like(mean_r)
        v_obs = v_obs * kpc_to_m
        sigma_v = sigma_v * kpc_to_m 
        sigma_z_only = sigma_z_only * kpc_to_m
        V_C = np.sqrt(v_obs**2 + VA2)
        V_C_spline = np.sqrt(v_obs**2 + VA_spline**2)


        return  mean_r, mean_acc_obs, mean_acc_bar, total_mean_acc_bar, v_obs, v_bar, v_tot, V_C, V_C_spline, sigma_v, h1_positions_rotated, h1_densities, rotated_points, sigma_z_only



    VA2 = mean_r * (sigma_v**2 * np.exp(mean_r / fitted_Rd)) / (fitted_Rd * (fitted_Rc + np.exp(mean_r / fitted_Rd)))

    #FIND V_A USING CUBIC SPLINE DERIVATIVE
    ...
    mask = VA2_spline < 0
    VA2_spline[mask] = 0  # Set negative values to zero


    VA2 = VA2 * kpc_to_m **2
    VA2_spline = VA2_spline * kpc_to_m**2
    v_obs = v_obs * kpc_to_m
    sigma_v = sigma_v * kpc_to_m  
    sigma_z_only = sigma_z_only * kpc_to_m

    for i in range(len(VA2_spline)):
        if np.isnan(VA2_spline[i]):
            VA2_spline[i] = 0

    V_C = np.sqrt(v_obs**2 + VA2)
    V_C_spline = np.sqrt(v_obs**2 + VA2_spline)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(gal)
        bincenters, mean_acc_obs, mean_acc_bar, total_mean_acc_bar, v_obs, v_bar, v_tot, gas_positions, gas_masses, rotated_points, sigma_z_only = V_plot(int(gal), ozy_file)

        V_plots = defaultdict(list)

        V_plots['bincenters'] = list(bincenters)
        #V_plots['mean_acc_obs'] = list(mean_acc_obs)
        #V_plots['mean_acc_bar'] = list(mean_acc_bar)
        #V_plots['total_mean_acc_bar'] = list(total_mean_acc_bar)
        V_plots['v_obs'] = list(v_obs)
        V_plots['v_bar'] = list(v_bar)
        V_plots['v_tot'] = list(v_tot)
        #V_plots['gas_positions'] = list(gas_positions)
        #V_plots['gas_masses'] = list(gas_masses)
        #V_plots['rotated_points'] = list(rotated_points)

        df = pd.DataFrame(V_plots)

        df.to_csv('/mnt/users/darnej/MPhys/V_plots/V_plot_for_'+str(gal)+'.csv')
