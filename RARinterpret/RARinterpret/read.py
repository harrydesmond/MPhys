...
class RARFrame:
    ...
    def read_rc(fpath):
        ...
        out = numpy.empty(data.shape[0], dtype=dtype)
        # Fill the array
    ...
    def add_fromlookup(data_rc, data_gal):
        ...
        # Preallocate an array
        out = numpy.empty(data_rc.size, dtype=descr_rc + descr_gal)
    ...
    def read_from_harry(self, dpath, lpath):
        """
        ...
        ----------
        dpath : str
            Path to the data file (e.g. ``samples_52_scatter_EFE2.npy``).
        lpath : str
            Path to the labels file (e.g. ``labels_52_scatter_EFE2.npy``).
